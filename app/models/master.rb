...
class Master
  ...
  class << self
    ...
    def ssh(*args)
      cmd = %w[ssh -o BatchMode=yes -o ConnectTimeout=10 -o ControlMaster=auto -o ControlPersist=10m]
      cmd += [ "-o", "ControlPath=#{Rails.root.join('tmp', 'ssh-%C')}" ]
      cmd += [ "-i", ENV["ARCHCI_SSH_KEY"], "-o", "IdentitiesOnly=yes" ] if ENV["ARCHCI_SSH_KEY"].present?
      out, err, status = Open3.capture3(*cmd, target, *args)
